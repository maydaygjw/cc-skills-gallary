...
```yaml
位置: .claude/.cache/db-schema.json
格式:
  database: string
...
| "运行xx报表" / "执行xx模板" | 本地模板 | 加载 .claude/db-patterns/ 下的 SQL 文件 |
...
```
用户：查询订单
AI：未找到数据库配置。请创建 .claude/db-config.yaml：
